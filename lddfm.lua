...
_resize: read term resize event and change x2 and y2 to match new width/height
...
makeMenu = function(_reszie, _x1,_y1,_x2,_y2,_path,_noFiles,_noFolders,_noCD,_noSelectFolders,_doHidden,_p,_canClose)
	local resize = false
	if _reszie then
		resize = true
	end
	...
	if _x1 == true then
		return false, "arguments: termResizeable, x1, y1, x2, y2, path, noFiles, noFolders, noCD, noSelectFolders, doHidden, palate, canClose" -- a little help
	end
	lddfm.setPalate(_p)
	...
	while true do
		...
		if evt[1] == "term_resize" and resize then
			x2, y2 = term.getSize()
